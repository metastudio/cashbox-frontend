...
import { ICustomersBalancesStatistic, ICustomersByMonthsStatistic, ICustomersStatistic } from 'services/statistic';
...
import ByMonthsChart from './by-months-chart';
import Chart from './chart';

import BalancesByProvider from './balances-by-provider';
import ExpenseByMonthsProvider from './expense-by-months-provider';
import ExpenseProvider from './expense-provider';
import IncomeByMonthsProvider from './income-by-months-provider';
...
class CustomersStatisticPage extends React.PureComponent<IProps> {
  ...
  private renderByMonthsChart = (stats: ICustomersByMonthsStatistic) => {
    if (!stats || !stats.data || stats.data.length === 0) {
      return <Alert bsStyle="info">No data</Alert>;
    }

    return <ByMonthsChart stats={ stats } />;
  }
      ...
      <>
        ...
        <Row>
          <Col xs={ 12 } sm={ 12 }>
            ...
            <IncomeByMonthsProvider orgId={ orgId } search={ location.search }>
              { this.renderByMonthsChart }
            </IncomeByMonthsProvider>
          </Col>
        </Row>
        ...
        <Row>
          <Col xs={ 12 } sm={ 12 }>
            ...
            <ExpenseByMonthsProvider orgId={ orgId } search={ location.search }>
              { this.renderByMonthsChart }
            </ExpenseByMonthsProvider>
          </Col>
        </Row>
