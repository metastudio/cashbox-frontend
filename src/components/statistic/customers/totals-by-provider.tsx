...
interface IDispatchProps {
  load: typeof loadTotalsByCustomersStatistic.request;
}

type IProps = IOwnProps & IStateProps & IDispatchProps;

class TotalsByCustomersStatisticProvider extends React.PureComponent<IProps> {
  private loadData = () => {
...
const mapState = (state: IGlobalState, props: IOwnProps): IStateProps => ({
  status:     selectTotalsByCustomersStatisticStatus(state),
  statistic:  selectTotalsByCustomersStatistic(state),
  period:     selectStatisticsQueryPeriod(props.search),
});

const mapDispatch = (dispatch: Dispatch): IDispatchProps => bindActionCreators(
  {
    load: loadTotalsByCustomersStatistic.request,
  },
  dispatch,
);
...
)(TotalsByCustomersStatisticProvider);
