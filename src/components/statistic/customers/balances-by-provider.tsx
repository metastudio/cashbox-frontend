...
interface IOwnProps {
  orgId:    ID;
  search:   string;
  children: (data: ICustomersBalancesStatistic) => React.ReactNode;
}

interface IStateProps {
  status:     Status;
  statistic:  ICustomersBalancesStatistic | null;
  period:     string | undefined;
}

interface IDispatchProps {
  load: typeof loadBalancesByCustomersStatistic.request;
}

type IProps = IOwnProps & IStateProps & IDispatchProps;

class BalancesByCustomersStatisticProvider extends React.PureComponent<IProps> {
  private loadData = () => {
...
const mapState = (state: IGlobalState, props: IOwnProps): IStateProps => ({
  status:     selectBalancesByCustomersStatisticStatus(state),
  statistic:  selectBalancesByCustomersStatistic(state),
  period:     selectStatisticsQueryPeriod(props.search),
});

const mapDispatch = (dispatch: Dispatch): IDispatchProps => bindActionCreators(
  {
    load: loadBalancesByCustomersStatistic.request,
  },
  dispatch,
);
...
)(BalancesByCustomersStatisticProvider);
